...
// Adiciona evento de clique para os botões de pagamento
...
document.getElementById('cnpj_cpf').addEventListener('input', function(event) {
    var input = event.target;
    var value = input.value;
    
    // Remove caracteres de formatação antes de reformatar
    var unformattedValue = value.replace(/[.-\/]/g, '');
    
    var formatted = formatarCPFouCNPJ(unformattedValue);
...
document.getElementById('phone').addEventListener('input', function(event) {
    ...
    // Limita o número de caracteres ao máximo de um número de telefone
    if (numericValue.length > 11) {
        numericValue = numericValue.slice(0, 11);
    }

    // Formatação para (XX) X XXXX-XXXX ou (XX) 9 XXXX-XXXX
    if (numericValue.length > 0) {
        formattedValue += '(' + numericValue.substring(0, 2) + ')';
        if (numericValue.length > 2) {
            formattedValue += ' ' + numericValue.charAt(2);
            if (numericValue.length > 3 && numericValue.charAt(2) === '9') {
                formattedValue += ' ' + numericValue.substring(3, 7) + '-';
                if (numericValue.length >= 10) {
                    formattedValue += numericValue.substring(7, 11);
                } else {
                    formattedValue += numericValue.substring(7);
                }
            } else {
                formattedValue += ' ' + numericValue.substring(3, 7) + '-';
                if (numericValue.length >= 10) {
